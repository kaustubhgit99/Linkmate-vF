...
export default function EditRoomPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg)' }}><div className="w-8 h-8 border-2 border-[#141414] border-t-transparent rounded-full animate-spin" /></div>

  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      <Navbar />
      <div className="pt-[64px] max-w-2xl mx-auto px-5 py-8">
        <Link href="/owner" className="flex items-center gap-1.5 text-[13px] text-[#5A5A58] hover:text-[#141414] mb-6 group transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <div className="card-lg overflow-hidden">
            <div className="px-7 py-5 border-b border-[#E4E4E2]">
              <h1 className="font-display text-[#141414] text-[1.4rem]">Edit Room</h1>
              <p className="text-[13px] text-[#5A5A58] mt-0.5 line-clamp-1">{room?.title}</p>
            </div>
            <form onSubmit={handleSave} className="px-7 py-6 space-y-5">
              <div><label className="field-label">Room Title *</label>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className="field-label">Area / Street *</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><label className="field-label">Beds</label>
                  <input type="number" min="1" value={form.num_beds} onChange={e => set('num_beds', e.target.value)} className="field" /></div>
                <div><label className="field-label">Rent (₹)</label>
                  <input type="number" min="0" value={form.rent_price} onChange={e => set('rent_price', e.target.value)} className="field" /></div>
              </div>
              <label className="flex items-center justify-between p-4 rounded-xl bg-[#F7F7F6] border border-[#E4E4E2] cursor-pointer">
                <div><p className="font-semibold text-[14px] text-[#141414]">Available for Rent</p><p className="text-[12px] text-[#A0A09E]">Toggle off if occupied</p></div>
                <Switch checked={form.is_available} onCheckedChange={v => set('is_available', v)} />
              </label>
              <div>
                <p className="text-[13px] font-semibold text-[#141414] mb-3">Amenities</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {AMENITIES_LIST.map(a => (
                    <label key={a} className={`flex items-center gap-2.5 p-2.5 rounded-xl cursor-pointer text-[13px] font-medium transition-all border ${form.amenities.includes(a) ? 'bg-[#141414] text-white border-[#141414]' : 'bg-white text-[#5A5A58] border-[#E4E4E2] hover:border-[#CECECE]'}`}>
          ...
          {room && (
            <div className="card-lg mt-5 p-7">
              <h3 className="font-semibold text-[15px] text-[#141414] mb-5">Room Photos</h3>
